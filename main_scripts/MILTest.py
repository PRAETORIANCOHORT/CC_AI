...
if __name__ == '__main__':
    ...
    os.mkdir(f'./{save_file}_{args.sample}_X{args.mag}/')
...
#     for option in ['train', 'valid']:
    ...
    for fd in range(5):
        os.mkdir(f'./{save_file}_{args.sample}_X{args.mag}/F{fd}/')
        if option == 'train':
...
#         epo = args.epo
        print('-'*30)
        epomax = np.max([int(i.split('.')[0]) for i in os.listdir(f'./checkpoints_{mg}X_{args.sample}_F{fd}/comment/') if i[-2:]=='pt'])+1
        ...
        for epoch in range(1, epomax):
            ...
            model.load_state_dict(torch.load(f'./checkpoints_{mg}X_{args.sample}_F{fd}/comment/{epo}.pt'))
            print(f'Testing: Mag-{mg}X-Epo{epo}')
            print(f'Model: {option}-X{mg}-{args.sample}-{epo}')
            print('-'*30)
            ...
            if args.local_rank == 0:
                ...
                result.to_csv(f'./{save_file}_{args.sample}_X{args.mag}/F{fd}/result-{option}-X{args.mag}-{epo}.csv')
...
#                 result = pd.read_csv(f'./2022-06-20-09-24-15_20226666KF_299_inveptionv3_X10/F{fd}/result-{option}-X{args.mag}-{epo}.csv')
        ...
        RESULTS_DF.to_excel(f'./{save_file}_{args.sample}_X{args.mag}/{option}_F{fd}_epochs_detail.xlsx', index=None)

        test_dataframe = pd.DataFrame({'test_label': test_label})
        test_dataframe.to_excel(f'./{save_file}_{args.sample}_X{args.mag}/test_label_{option}_F{fd}.xlsx', index=None)
